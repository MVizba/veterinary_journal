...
final class AppointmentController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        $drug = new AppointmentDrug();   //
        $appointment->addDrug($drug);    //
        $examination = new AppointmentExamination();
        $appointment->addExamination($examination);

        $form = $this->createForm(AppointmentType::class, $appointment);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            foreach ($appointment->getExaminations() as $examination) {
                $examination->setAppointment($appointment);
            }
            foreach ($appointment->getDrugs() as $drug) {
    ...
    public function show(Appointment $appointment): Response
    {
        return $this->render('appointment/show.html.twig', [
            ...
            'examinations' => $appointment->getExaminations(),
